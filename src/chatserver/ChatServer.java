    ...
    private static class Handler implements Runnable {
        ...
        public void run() {
            try {
                ...
                for (String active_user : names) {
                    out.println("ADD_USER" + active_user);
                }

                for (PrintWriter writer : writers.values()) {
                    writer.println("ADD_USER" + name);
                }
                ...
                while (true) {
                    ...
                    synchronized (writers) {
                        String message = "";
                        HashSet<PrintWriter> writersToBeWritten = new HashSet<>();
                        String[] split_input_array = input.split(">>");
                        
                        if (split_input_array.length == 2) {
                            String[] receivers_array = split_input_array[0].split("|");
                            message = split_input_array[1];
                            for (String receiver : receivers_array) {
                                writersToBeWritten.add(writers.get(receiver));
                            }
                            for (PrintWriter writer : writersToBeWritten) {
                                writer.println("MESSAGE " + name + ": " + message);
                            }

                            // writersToBeWritten.add(writers.get(receivers_array));
                        } else {
                            message = input;
                            writersToBeWritten.addAll(writers.values());
                            for (PrintWriter writer : writersToBeWritten) {
                                writer.println("MESSAGE " + name + ": " + message);
                            }
                        }
